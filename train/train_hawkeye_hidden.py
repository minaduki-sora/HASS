...
# --- 模型定义 ---
class LSTMPolicyNetHidden(nn.Module):
...
class SharedStatesHiddenDataset(Dataset):
    def __init__(self, dataset, max_len=7):
        ...
        for sample in tqdm(dataset, desc="Loading dataset"):
            ...
            for i in range(1, max_len + 1):
                hidden_b64 = sample[f'eagle_{i}_hidden']
                # 解码base64编码的hidden states
                buffer = io.BytesIO(base64.b64decode(hidden_b64))
                hidden_np = np.load(buffer).astype(np.float32)
                hidden_seq.append(hidden_np)
            # 对于第一个前向，hidden是[hidden_dim]，对于后续前向，hidden是[top_k, hidden_dim]
            # 我们只取top_k=10的第一个元素
            hidden_seq = [h[0] if h.ndim == 2 else h for h in hidden_seq]
...
def collate_fn_hidden(batch):
    hidden_states = torch.tensor([x["hidden_states"] for x in batch], dtype=torch.float32)
    ...
    return hidden_states, stop_probs, avg_acc_lens_op
...
def cal_avg_len(model, data_loader, eatime, speed_matrix, num_samples=10):
    ...
    with torch.no_grad():
        for hidden_states, stop_probs, avg_acc_lens_op in tqdm(data_loader, desc="Calculating Stats", leave=False):
            hidden_states, stop_probs, avg_acc_lens_op = hidden_states.to(device), stop_probs.to(device), avg_acc_lens_op.to(device)
            ...
            for _ in range(num_samples):
                # 1. 每次都从模型采样新的动作
                hidden = None
                actions, _, _ = model.act(hidden_states, hidden) # [B, T]
...
def evaluate_loss(model, data_loader, G):
    ...
    total_loss = 0
    with torch.no_grad():
        for hidden_states, stop_probs, _ in tqdm(data_loader, desc="Evaluating Loss", leave=False):
            hidden_states, stop_probs = hidden_states.to(device), stop_probs.to(device)
            logits, _ = model(hidden_states)
...
def train_and_evaluate(params, train_loader, test_loader, project_root):
    ...
    # 创建输出目录
    output_dir = os.path.join(project_root, f"output/{params['bench_name']}/{params['base_model_name']}/{params['td']}_hidden")
    ...
    for epoch in epoch_pbar:
        ...
        for hidden_states, stop_probs, _ in batch_pbar:
            hidden_states, stop_probs = hidden_states.to(device), stop_probs.to(device)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--config', type=str, default='eagle/train/train_debug_hidden.json', help='Path to the configuration JSON file.')
    ...
    train_dataset = SharedStatesHiddenDataset(ReplayDataset["train"], max_len=max_len)
    test_dataset = SharedStatesHiddenDataset(ReplayDataset["test"], max_len=max_len)
    
    train_loader = DataLoader(train_dataset, batch_size=batch_size, shuffle=True, collate_fn=collate_fn_hidden)
    test_loader = DataLoader(test_dataset, batch_size=batch_size, shuffle=False, collate_fn=collate_fn_hidden)
